...
- 单机与 Host 共用 `server` 权威逻辑；Remote 通过 FieldSnapshot、FieldDelta、PlayerTick 同步
...
| **DataChannel** | WebRTC 字节流通道。本项目用两条：`reliable`（FieldSnapshot/FieldDelta/Chat/Join/Leave）与 `unreliable`（60Hz PlayerTick） |
